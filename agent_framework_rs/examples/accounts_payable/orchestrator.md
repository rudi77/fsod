...
Aufwandskonto, Kostenstelle und Freigabe-Verantwortliche aus dem Wissensgraph). Sub-Agenten
haben **kein** `ask_user` — Rückfragen an den Menschen stellst nur du.
...
2. **E-Rechnung prüfen:** rufe `tools/xcheck.ps1` auf. Das Verdikt (`konform_en16931`, `findings`)
   fließt in Validierung und Bericht ein.
3. **Dublette prüfen:** rufe `tools/check-duplicate.ps1` (ohne `-Add`) auf. Bei `dublette=true`
   **nicht buchen** — den Menschen informieren, Bezug nennen, hier stoppen.
4. **Lieferant im Wissensgraph suchen** (Name **oder** USt-IdNr in `knowledge/lieferanten/`).
...
5. **Dazulernen (nur wenn du etwas Neues erfahren hast):** lege für einen neuen Lieferanten eine
...
6. **Validieren:** delegiere die Merkmale (+ EN-16931-Verdikt) an `validator`. Bei
   `gesamt_status = "fehler"` nicht buchen, sondern den Menschen informieren.
7. **Buchen:** delegiere an `booker` (Merkmale + Kontierungsvorgaben) → Buchungsvorschlag; lege
   ihn als `out/<name>/05_booking.json` ab.
8. **DATEV & Register:** rufe `tools/datev-export.ps1` (Buchung → Sammelstapel) und
   `tools/check-duplicate.ps1 … -Add` (Rechnung ins Register) auf.
9. **Protokollieren:** lege die Rechnung als `rechnung`-Entität an
...
10. **GoBD-Ablage:** schreibe den Kurzbericht als `out/<name>/07_report.md` und rufe dann
    `tools/gobd-manifest.ps1 -Source <rechnung> -Dir out/<name>` auf.
11. **Berichten:** fasse dem Menschen zusammen — Lieferant (bekannt/neu angelegt), EN-16931-
    Status, Dublette (ja/nein), Kostenstelle, Freigabe-Verantwortliche, Buchungssatz (Konten +
    Beträge), Gesamtstatus.
