...
# --------------------------------------------------------------------- Setup
def build_llm(provider: str = "auto", force_demo: bool = False):
    ...
    if force_demo or provider == "demo":
        return select_llm(force_demo=True)
    if provider == "azure":
        return azure_from_env(), "azure"
    if provider == "openai":
        return openai_from_env(), "openai"
    return select_llm(force_demo=False)  # auto: errät Provider, sonst Demo-Fallback


def build_agent(args) -> tuple[Agent, ToolRegistry, Plan, Skills | None, dict]:
    """Stellt den Agenten zusammen und gibt `(agent, tools, plan, skills, roles)` zurück.

    Zwei Betriebsarten in EINER CLI:
    - **Echter LLM** (Azure/OpenAI): der volle Coding-Agent — Sandbox-Tools, Skills,
      Plan und das `task`-Tool für Sub-Agenten. `roles` = eingebaute `ROLES` + ggf.
      via `--agents` geladene Custom-Rollen (für `/agents`).
    - **Demo** (kein API-Key / `--demo`): der netzfreie Demo-LLM mit einem kleinen
      Werkzeugkasten — sofort lauffähig, ohne Sandbox/Sub-Agenten-Overhead.
    """
    llm, label = build_llm(args.provider, force_demo=getattr(args, "demo", False))
    print(f"{C.GRAY}» Modell: {label}{C.RESET}", file=sys.stderr)

    # Demo-Modus: schlanker, netzfreier Agent (wie die installierte Executable ohne Key).
    if label.startswith("demo"):
        from .demo import demo_tools
        tools = demo_tools()
        agent = Agent(llm, tools=tools, strategy=args.strategy, max_steps=args.max_steps)
        return agent, tools, Plan(), None, {}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-V", "--version", action="version", version=f"agentkit {__version__}")
    return p
